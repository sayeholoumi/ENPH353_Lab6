...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        vertical_division = int(width/10)
    ...
    #finding center of line
        ...
        #line_pos gives the center of the line
        line_pos = (firstpoint+int(x/2))#+30

        i=0
        
        while (vertical_division*(i+1)!=width):
            if ((vertical_division*i)<=line_pos) and (line_pos<=(vertical_division*(i+1))) and (line_found):
                state[i]=1
            else: 
                state[i]=0
            i = i+1

        if all(np.array(state) == 0):
        ...
        cv2.putText(img=cv_image, text=str(state), org=(20, 20), 
                    fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.4, 
                    color=(0, 255, 0),thickness=1)
        cv2.imshow("my image", cv_image)
        cv2.waitKey(3)
        #init_w = 0
    ...
    def step(self, action):
        ...
        if action == 0:  # FORWARD
            vel_cmd.linear.x = 0.4 #0.4
            vel_cmd.angular.z = 0.0
        elif action == 1:  # LEFT
            vel_cmd.linear.x = 0.0
            vel_cmd.angular.z = 0.8 #0.5
        elif action == 2:  # RIGHT
            vel_cmd.linear.x = 0.0
            vel_cmd.angular.z = -0.8 #0.5
